...
In this problem, you are given a single integer \`n\`. Your task is to determine whether the number is an **Armstrong number**.

An **Armstrong number** of \`d\` digits is an integer such that the sum of its digits each raised to the power \`d\` is equal to the number itself.
...
- Breaking an integer into its digits
- Performing power operations
- Looping constructs and conditionals

Your program should read an integer from standard input and print either \`Yes\` or \`No\` based on whether it's an Armstrong number.

---

Additional Notes:
-----------------
- You must not use hardcoded checks for specific numbers.
- Ensure your solution works for any number between 1 and 100000.
- Your output should **exactly** be \`Yes\` or \`No\` with no extra spaces or lines.
- Avoid stringifying outputs like "The number is Armstrong".

Good luck, and happy coding!
  `,
  input: "A single integer n (1 ≤ n ≤ 100000)",
  output: `"Yes" if the number is an Armstrong number, otherwise "No"`,
